...
def main(args, data_path, output_folder):
    ...
    cluster, center = kmeans.kmeans(features, num_clusters)
    ...
    #     def __init__(self):
    #         super().__init__()
    #         self.backbone = torch.nn.Sequential(torch.nn.Linear(71936, 1280), torch.nn.ReLU(), torch.nn.Linear(1280, 128))
    #
    ...
    class _Model(torch.nn.Module):
        def __init__(self, in_dim, hidden_dim, out_dim, num_heads, dropout):
            ...
            self.layer1 = GATConv(in_dim, hidden_dim, num_heads, feat_drop=dropout, attn_drop=dropout,allow_zero_in_degree=True)
            self.layer2 = GATConv(hidden_dim * num_heads, out_dim, 1, feat_drop=dropout, attn_drop=dropout,allow_zero_in_degree=True)
        ...
        def forward(self, g, x):
            ...
            h = h.flatten(1)
            h = self.layer2(g, h)
            return h.squeeze(1)  # shape: (num_nodes, out_dim)

    # Models = GAT(features.shape[1], hidden_dim=32, out_dim=128, num_heads=4, dropout=0.5).to(device)

    backbone = _Model(
                  features.shape[1], hidden_dim=32, out_dim=128, num_heads=4, dropout=0.5)
    # Models = GAT(features.shape[1], hidden_dim=32, out_dim=128, num_heads=4, dropout=0.5).to(device)
...
if __name__ == '__main__':
    ...
    test_result = '/home/user/wyh/DC_contribution/finally_project/code/HAN/unsupervised_output/GAT'
    output_folder ='/home/user/wyh/DC_contribution/finally_project/code/HAN/unsupervised_output/GAT'
    os.makedirs(test_result, exist_ok=True)
    os.makedirs(output_folder,exist_ok=True)

    data_folder = '/home/user/wyh/DC_contribution/finally_project/code/RFmaster/pkl_files/mlkit_unlabel/Experiment_20250221-191646_GAT'
    pkl_files = [f for f in os.listdir(data_folder) if f.endswith('.pkl')]

    # test_result = '/home/user/wyh/DC_contribution/finally_project/code/HAN/unsupervised_output/FC/result/mlkit'
    # output_folder = '/home/user/wyh/DC_contribution/finally_project/code/HAN/unsupervised_output/FC/save_model/mlkit'
    # os.makedirs(test_result, exist_ok=True)
    # os.makedirs(output_folder, exist_ok=True)
    #
    # # Load all .pkl files in the folder and train Models
    # data_folder = '/home/user/wyh/DC_contribution/finally_project/code/RFmaster/pkl_files/mlkit_unlabel/Experiment_20250221-191646'
    # pkl_files = [f for f in os.listdir(data_folder) if f.endswith('.pkl')]
